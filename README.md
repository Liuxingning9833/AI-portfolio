<p align="center">
  <img src="https://img.shields.io/badge/OS-Windows%2010-blue?logo=windows" alt="OS">
...
<p align="center"><strong>个人 AI 应用作品集 · 六个完整项目</strong></p>
...
六个项目覆盖：RESTful API 设计、RAG 知识库、流式输出、批量任务处理、JSON Function Calling 智能体、Meta Prompting 提示词工程。
...
- **`meta-prompt/`** — AI 提示词教练，输入粗糙提示词 → 输出专业优化版
  - `main.py` — FastAPI 服务入口，含优化/诊断/模板三种模式
  - `ui.html` — 前端交互界面，评分展示 + 一键复制
  - `requirements.txt` — Python 依赖

- **`学习计划.md`** — Python + ACP 备考 + 求职计划
- **`screenshots/`** — 项目截图存放目录
...
### 6. Meta Prompting — AI 提示词教练

让大模型成为你的 Prompt Engineering 教练。输入粗糙描述 → AI 分析 + 优化 → 输出专业级提示词。

- **提示词优化** — 原始提示词 → 结构化的专业 Prompt，含评分和改进建议
- **诊断分析** — 分析现有提示词的优缺点
- **模板生成** — 根据需求描述生成 Prompt 模板
- **一键复制** — 优化结果直接复制使用
...
cd meta-prompt
uvicorn main:app --host 127.0.0.1 --port 8864
```
...
| Meta Prompting | `http://127.0.0.1:8864` |
